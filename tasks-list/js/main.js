...
(function () {
  ...
  var formTemplate = function () {
    ...
    return (
      ...
      txtTask +
      txtOwner +
      "<button type='button' class='btn btn-default btn-xs' onclick='APP.saveTask(this);'><span class='save glyphicon glyphicon-ok' aria-hidden='true'></span></button>&nbsp;<button type='button' class='btn btn-default btn-xs' onclick='APP.resetAddTask();'><span class='cancel glyphicon glyphicon-remove' aria-hidden='true'></span></button>" +
      divAlert +
      "</div>"
    );
  };
  ...
  var taskTemplate = function (id, parentId, desc, owner, level, childs) {
    ...
    var col_arrow =
      "<div class='col-xs-" +
      cls_col_1 +
      " text-right'><button type='button' class='" +
      cls_hidden +
      " btn btn-default btn-xs arrow' onclick='APP.toggleSubTasks(this);'><span class='arrow glyphicon glyphicon-menu-right' aria-hidden='true'></span></button></div>";
    var col_desc =
      "<div class='col-xs-" +
      cls_col_2 +
      "'><span class='task'>" +
    ...
    if (level < 6) {
      col_AddBtn =
        "<button type='button' class='btn btn-default btn-xs add' onclick='APP.moveTextBox(this);'><span class='glyphicon glyphicon-plus' aria-hidden='true'></span></button>";
    }
    var col_DeleteBtn =
      "<button type='button' class='btn btn-default btn-xs delete' onclick='APP.deleteTask(this);''><span class='glyphicon glyphicon-trash' aria-hidden='true'></span></button>";
    return (
      "<div class='row task' id='task_id_" +
      id +
      "'  data-task-id='" +
      id +
      "' data-parent-id='" +
  ...
  APP.saveTask = function (btnSave) {
    ...
    var parentTask =
      parentTaskId === 0 ? $todoList : $("#task_id_" + parentTaskId);
  ...
  APP.moveTextBox = function (btnObj) {
    ...
    const lastOwner = !!parentObj.find("span.owner")
      ? parentObj.find("span.owner").text()
      : "";

    $(saveObj).addClass("col-xs-offset-" + level - 1);
  ...
  APP.toggleSubTasks = function (btnObj) {
    ...
    if ($('.row[data-parent-id="' + parentTaskId + '"]').length === 0) {
      ...
      $(btnObj)
        ...
        .attr(
          "class",
          $(btnObj).find("span").attr("class").replace(/right/, "down")
        );
    } else {
      ...
      $(btnObj)
        ...
        .attr(
          "class",
          $(btnObj).find("span").attr("class").replace(/down/, "right")
        );
    }
  };
  ...
  APP.resetAddTask = function () {
    var offsetClass = $(saveObj)
      .attr("class")
      .match(/col-xs-offset-\d/);
  ...
  var addTaskItem = function (parentObj) {
    ...
    if (taskDesc.length === 0) {
      $("#msg")
        .text("Task must contain a description!")
        .fadeIn(200)
        .fadeOut(3500);
      return;
    }
    if (taskOwner.length === 0) {
      $("#msg").text("Task must contain an owner!").fadeIn(200).fadeOut(3500);
      return;
    }
  ...
  var nextTaskId = function () {
    if (taskslist.length > 0) {
      taskIdx = Math.max.apply(
        Math,
        $.map($(taskslist), function (o) {
          return o.id;
        })
      );
    }
  ...
  var resetForm = function (lastOwner = "") {
    $("#txtTaskDesc").val("").focus();
    $("#txtOwner").val(lastOwner);
  };
